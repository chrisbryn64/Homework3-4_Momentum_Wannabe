...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let getUserNameValues = function (e) {
    ...
    // Check if the keypress is "Enter" key for setting userName value
    if (e.keyCode === 13) {
      ...
      /*This is how we set the thingy to show only one time */
  ...
  // Check if the userName is stored or not
  let checkUserNameValue = function () {
    // check if localStorage has userName or not
    if (localStorage.getItem('userDetails') === null) {
      //if it is then display first time block
      ...
      // if it's not then display a greet message
  ...
  let setGreet = function (elm) {
    ...
    //set greet message according to hours
    if (hours >= 4 && hours <= 12) {
  ...
  // Functions to set random quotes
  ...
  //Select Quote text Element
  let quoteText = document.querySelector('.quote_text');
  
  // Select Quote Author Element
  ...
  //reset all information if reset all information button is clicked
  let resetData = function () {
    localStorage.removeItem('userDetails');
    localStorage.removeItem('todoTasks');
    alert('Your wish has been granted. Farewell.');
  }
